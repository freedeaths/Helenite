...
export const getVaultConfig = (): VaultConfig => {
  return {
    baseUrl: NEW_VAULT_CONFIG.VAULT_PATH,
    vaultsRoot: NEW_VAULT_CONFIG.VAULTS_ROOT,
    indexFile: NEW_VAULT_CONFIG.indexFile,
    excludedFolders: NEW_VAULT_CONFIG.excludedFolders,
    excludedExtensions: NEW_VAULT_CONFIG.excludedExtensions,
    search: NEW_VAULT_CONFIG.search
  };
};
...
export const isFolderExcluded = (folderName: string): boolean => {
  return NEW_VAULT_CONFIG.excludedFolders.includes(folderName);
};
...
export const isFileExcluded = (fileName: string): boolean => {
  return NEW_VAULT_CONFIG.excludedExtensions.some(ext => fileName.endsWith(ext));
};
...
export const isPathInExcludedFolder = (filePath: string): boolean => {
  return NEW_VAULT_CONFIG.excludedFolders.some(excludedFolder =>
