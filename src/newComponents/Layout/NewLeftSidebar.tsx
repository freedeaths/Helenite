...
export function NewLeftSidebar() {
  const { activeMobileTab, setActiveMobileTab, activeLeftPanel, setActiveLeftPanel, isMobile, isTablet } = useNewUIStore();
    ...
    return (
      <div className="h-full flex flex-col bg-[var(--background-secondary)]">
        ...
        <div className="flex-1 overflow-hidden" style={{ padding: '1rem' }}>
          {activeLeftPanel === 'files' && <NewFileExplorer />}
          {activeLeftPanel === 'tags' && <NewTagsPanel />}
        </div>
      </div>
    );
  }
