...
interface NewMobileDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  side: 'left' | 'right';
  title: string;
...
export function NewMobileDrawer({ isOpen, onClose, side, title, children }: NewMobileDrawerProps) {
  // Handle escape key
  useEffect(() => {
    if (!isOpen) return;
      ...
      >
        <div className="h-full flex flex-col">
          ...
          <div className="flex items-center justify-between p-4 border-b border-[var(--background-modifier-border)]">
            <h2 className="text-sm font-medium text-[var(--text-normal)]">{title}</h2>
            <ActionIcon
              onClick={onClose}
              variant="subtle"
              color="gray"
