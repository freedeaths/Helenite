...
import { NewRightSidebar } from './NewRightSidebar';
import { NewStatusBar } from './NewStatusBar';
import { NewResizeHandle } from './NewResizeHandle';
import { NewMobileDrawer } from './NewMobileDrawer';
import { NewLeftRibbon } from './NewLeftRibbon';
import { NewUnifiedMobileDrawer } from './NewUnifiedMobileDrawer';
import { NewMobileDropdownMenu } from './NewMobileDropdownMenu';
...
export function NewAppLayout() {
  ...
  return (
    <div data-testid="app-layout" className="h-screen w-screen flex flex-col overflow-hidden bg-[var(--background-primary)] text-[var(--text-normal)]">
      ...
      {/* 移动端模拟正常显示，真机不显示 */}
      {!isMobile && <NewStatusBar />}

      {/* Mobile Navigation - 使用老版本风格的下拉菜单 */}
      {isMobile && <NewMobileDropdownMenu />}
