...
import { getCurrentRoute, navigateToFile, navigateToWelcome, navigateToGlobalGraph, type ParsedRoute } from './routeUtils';

/**
 * 新架构路由管理 Hook
 * 完全复制原版路由逻辑，适配新的 store 系统
 */
export function useNewHashRouter() {
  const { setActiveFile } = useNewVaultStore();
  const { setMainContentView } = useNewUIStore();
