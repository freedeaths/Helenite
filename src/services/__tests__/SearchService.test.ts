...
describe('SearchService', () => {
  ...
  describe('searchContent - 全文搜索', () => {
    ...
    it('应该能够高亮搜索结果', async () => {
      ...
      expect(match.highlighted).toContain(
        '<span class="search-result-file-matched-text">knowledge</span>'
      );
    });
    ...
    it('应该支持不区分大小写的搜索', async () => {
      ...
      expect(
        results.some((r) => r.matches.some((m) => m.content.toLowerCase().includes('react')))
      ).toBe(true);
    });
  ...
  describe('searchByTag - 标签搜索', () => {
    it('应该能够搜索标签（带 # 前缀）', async () => {
      ...
      expect(results.some((r) => r.fileName === 'React Guide')).toBe(true);
      expect(results.some((r) => r.matches.some((m) => m.content.includes('Tag: #react')))).toBe(
        true
      );
    });
  ...
  describe('工具方法', () => {
    it('highlightSearchResults 应该正确高亮文本', () => {
      ...
      expect(highlighted).toBe(
        'This is a <span class="search-result-file-matched-text">React</span> application'
      );
    });
    ...
    it('highlightSearchResults 应该处理特殊字符', () => {
      ...
      expect(highlighted).toBe(
        'Price: <span class="search-result-file-matched-text">$10.99</span>'
      );
    });
  ...
  describe('错误处理', () => {
    ...
    it('应该处理文件元数据获取错误', async () => {
      (mockMetadataService.getFileMetadata as Mock).mockRejectedValue(
        new Error('File metadata error')
      );
