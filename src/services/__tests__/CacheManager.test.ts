...
describe('CacheManager', () => {
  ...
  describe('Cache Warmup', () => {
    ...
    it('should handle warmup errors gracefully', async () => {
      ...
      // 应该不抛出错误，优雅处理失败的文件
      await expect(cacheManager.warmupCache(errorStorage, commonFiles)).resolves.not.toThrow();
    });
  });
  ...
  describe('Reusable Service Proxies', () => {
    ...
    it('should create generic cached service', () => {
      ...
      const cachedTestService = cacheManager.createCachedService(testService, 'test', cacheConfig);
